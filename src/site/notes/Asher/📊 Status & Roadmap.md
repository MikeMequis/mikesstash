...
| Install wizard chrome      | 🔜 Deferred | Fuller welcome / stepper polish              |
...
Optional polish (not unfinished migration work): install wizard stepper chrome; Content Patcher UI only when a backend exists.
...
Polimento opcional (não é migração incompleta): chrome do assistente de instalação; UI de Content Patcher só quando houver backend.
