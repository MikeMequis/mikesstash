...
| Avalonia manager UI                             | 🔜 Planned  | Cross-platform user interface, along with in-app updates for portable version |
...
Porting the remaining gameplay patches from [DustAetPatchingPlatform](https://github.com/GMMan/DustAetPatchingPlatform) into Asher's module architecture (`IAsherPreInitModule`, `IAsherPatchModule`, lifecycle hooks), and validating each one against the real game.
...
- **Avalonia-based manager UI** → share the C#/.NET stack instead of shipping a Chromium runtime.
...
| UI Avalonia do gerenciador                      | 🔜 Planejado | Interface visual multiplataforma, junto com atualizações in-app da versão portátil  |
...
Portar os patches de gameplay restantes do [DustAetPatchingPlatform](https://github.com/GMMan/DustAetPatchingPlatform) para a arquitetura de módulos do Asher (`IAsherPreInitModule`, `IAsherPatchModule`, hooks de ciclo de vida) e validar cada um no jogo real.
...
- **Rich Presence do Discord personalizável** → alternar informações visíveis a respeito da sessão do jogo ativa
- **UI de gerenciador baseada em Avalonia** → compartilhamento de stack C#/.NET em vez de embarcar um runtime Chromium.
